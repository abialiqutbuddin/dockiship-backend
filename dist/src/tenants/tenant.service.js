...
let TenantService = class TenantService {
    ...
    async createForUser(userId, dto) {
        ...
        return await this.prisma.$transaction(async (tx) => {
            ...
            if (allPerms.length === 0) {
                throw new common_1.BadRequestException('No permissions found. Run `npx prisma db seed` before creating tenants.');
            }
    ...
    async hardDeleteTenant(tenantId, requesterUserId) {
        ...
        if (!ownerMembership?.isOwner) {
            throw new common_1.ForbiddenException('Only tenant owners can delete the tenant');
        }
